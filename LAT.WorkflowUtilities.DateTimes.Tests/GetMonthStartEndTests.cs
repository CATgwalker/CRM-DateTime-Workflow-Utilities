...
namespace LAT.WorkflowUtilities.DateTimes.Tests
{
    ...
    public class GetMonthStartEndTests
    {
        ...
        [TestMethod]
        public void GetMonthStartEnd_Valid_Date1_User_Local()
        {
            //Arrange
            ...
            //Act
            var result = xrmFakedContext.ExecuteCodeActivity<GetMonthStartEnd>(workflowContext, inputs);

            //Assert
            Assert.AreEqual(expected1, result["MonthStartDate"]);
            Assert.AreEqual(expected2, result["MonthEndDate"]);
        }

        [TestMethod]
        public void GetMonthStartEnd_Valid_Date2_User_Local()
        {
            //Arrange
            XrmFakedWorkflowContext workflowContext = new XrmFakedWorkflowContext();
            ...
            xrmFakedContext.Initialize(new List<Entity> { userSetting });
            xrmFakedContext.CallerId = new EntityReference("systemuser", userSetting.GetAttributeValue<Guid>("systemuserid"));
            var fakeLocalTimeFromUtcTimeRequestExecutor = new FakeLocalTimeFromUtcTimeRequestExecutor(new DateTime(2015, 2, 3, 11, 0, 0));
            xrmFakedContext.AddFakeMessageExecutor<LocalTimeFromUtcTimeRequest>(fakeLocalTimeFromUtcTimeRequestExecutor);
        ...
        [TestMethod]
        public void GetMonthStartEnd_Valid_Date1_Utc()
        {
            //Arrange
            XrmFakedWorkflowContext workflowContext = new XrmFakedWorkflowContext();
